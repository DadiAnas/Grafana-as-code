...
locals {
  ...
  # Environment-specific rules override shared ones with same name
  # =============================================================================
  shared_alert_rules = try(yamldecode(file("${path.module}/config/shared/alerting/alert_rules.yaml")), { alert_rules = [] })
  env_alert_rules    = try(yamldecode(file("${path.module}/config/${local.env}/alerting/alert_rules.yaml")), { alert_rules = [] })
  
  # Create maps by name for merging (env overrides shared)
  shared_ar_map = { for ar in local.shared_alert_rules.alert_rules : ar.name => ar }
  env_ar_map    = { for ar in local.env_alert_rules.alert_rules : ar.name => ar }
  merged_ar_map = merge(local.shared_ar_map, local.env_ar_map)
  
  alert_rules_config = {
    alert_rules = values(local.merged_ar_map)
  }
